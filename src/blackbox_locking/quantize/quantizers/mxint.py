...
def _check_shape_mxint(x: torch.Tensor, block_size: int, block_axis: int):
    ...
    if x.ndim == 1:
        ...
        assert block_axis in [
            0,
            1,
            -1,
            -2,
        ], "block_axis must be 0, 1, -1, or -2 for 2D tensor"
    elif x.ndim == 3:
        ...
        assert block_axis in [
            ...
            -2,
            -1,
        ], "block_axis must be 1, 2, -2, or -1 for 3D tensor"
    else:
        raise NotImplementedError("Only support 1D, 2D tensor, and 3D activation tensor")
    return True
...
def mxint_quantizer(x: torch.Tensor, width: int, block_size: int, block_axis: int):
    ...
    if len(ori_shape) > 3:
        ...
        x = x.reshape(-1, *ori_shape[-2:])

    x_q = MXINTQuantize.apply(x, width, block_size, block_axis)
