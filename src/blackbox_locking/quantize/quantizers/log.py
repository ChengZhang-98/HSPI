...
from .utils import q_clamp, q_round
...
def _log_quantize(
    ...
    exponent = q_clamp(q_round(torch.log2(value)), exponent_min, exponent_max)
...
class LogQuantize(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: Tensor, width: int, exponent_bias: int | Tensor | ndarray | None):
...
def log_quantizer(
    x: Tensor | ndarray,
    width: int,
    exponent_bias: int | Tensor | ndarray | None = None,
):
    """
