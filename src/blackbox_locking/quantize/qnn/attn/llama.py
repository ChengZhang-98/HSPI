...
class QLlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int], q_config: dict):
        super().__init__()
        self.config = config
...
def create_q_llama_attn(ori_m: LlamaAttention, q_config: dict):
    new_m = QLlamaAttention(ori_m.config, ori_m.layer_idx, q_config)
    new_m.load_state_dict(ori_m.state_dict())
...
def quantize_llama_layer_to_built_in_dtype(model: LlamaForCausalLM, dtype: torch.dtype):
    ropes = {}
